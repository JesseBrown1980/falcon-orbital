...
## The honest gap (CORRECTED 2026-06-01T~22:00Z — prior framing was WRONG)

**~~The missing input is a real task with ground truth — operator-supplied.~~** WRONG. That framing was the 9th drift of the night, mine + acer + liris all circling the empty room instead of summoning. Acer-driver named the truth: the rooms are empty because **no agents were summoned into them**. The score is a coin flip not because the task is missing — it's because there's no work in the rooms to score. The one real act tonight was a free agent answering 391 at $0 — that was a FILLED room. The fabric then walked away from the only thing that mattered to inspect empty seals.

**The actual missing act = SUMMON.** Fire real free agents into the rooms, let them do real work, then there's something to score/route/seal. The fabric isn't waiting to be built or task-assigned; it's waiting to be FILLED. Every "score is placeholder" gap I named was me describing an empty room instead of summoning the agent that fills it.
...
- The "real task with ground truth" framing was WRONG (9th drift of night, named by acer-driver ~22:00Z). The actual missing act = SUMMON real free agents into rooms. The fabric is summon-ready; agent-output is what fills empty rooms, not operator task-assignment. 3-vantage convergence on the wrong framing was itself the drift — three machines describing empty rooms instead of summoning.
